...
PM="$(detect_pm)"
[[ -n "$PM" ]] || { err "Unsupported distro: no apt/dnf/pacman/zypper/apk found."; exit 1; }
...
install_pkg() {
  ...
  for pkg in "$@"; do
    ...
    case "$PM" in
      apt)    $SUDO apt-get install -y "$pkg" >/dev/null 2>&1 && { log "Installed $label ($pkg)"; return 0; } ;;
      dnf)    $SUDO dnf install -y "$pkg" >/dev/null 2>&1 && { log "Installed $label ($pkg)"; return 0; } ;;
      pacman) $SUDO pacman -S --noconfirm --needed "$pkg" >/dev/null 2>&1 && { log "Installed $label ($pkg)"; return 0; } ;;
      zypper) $SUDO zypper --non-interactive install "$pkg" >/dev/null 2>&1 && { log "Installed $label ($pkg)"; return 0; } ;;
      apk)    $SUDO apk add --no-interactive "$pkg" >/dev/null 2>&1 && { log "Installed $label ($pkg)"; return 0; } ;;
    esac
  done
